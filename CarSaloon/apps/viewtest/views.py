...
def hello(request):
        ...
        context={
            'name':'Hell Max Jokar & Django:This message is  from View Side: def hello(request):'
...
# Class Base Generic Views: Detail, Create,Edit Delete, "CRAD "operation :
...
# instead def and etc by writing 2 lines  code we can handle everything here:
...
class PostList(ListView): #As Default listView puts its list inside place or context named: object_list(in html)
        model=Post
# to handle The error Appears after we Post ,we should use:SuccessURL! above mentioned!
...
class GenericClass1(ListView):
    ...
    context_object_name='posts' #Our datas dont go to Object_list , This way  goes to  post and not object_list:
